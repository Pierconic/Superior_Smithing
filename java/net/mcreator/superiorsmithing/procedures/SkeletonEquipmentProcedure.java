				...
				if (ominousness == 2) {
					...
					if (type == 0) {
						if (weaponenchanted == true) {
							if (entity instanceof LivingEntity _entity) {
								ItemStack _setstack = (EnchantmentHelper.enchantItem(world.getRandom(), new ItemStack(SuperiorsmithingModItems.GILDED_DIAMOND_SWORD.get()), 20,
										(false)
						...
						if (weaponenchanted == true) {
							if (entity instanceof LivingEntity _entity) {
								ItemStack _setstack = (EnchantmentHelper.enchantItem(world.getRandom(), new ItemStack(SuperiorsmithingModItems.GILDED_DIAMOND_AXE.get()), 20,
										(false)
												? world.registryAccess().registryOrThrow(Registries.ENCHANTMENT).holders().map(reference -> (Holder<Enchantment>) reference)
												: world.registryAccess().registryOrThrow(Registries.ENCHANTMENT).getTag(EnchantmentTags.IN_ENCHANTING_TABLE).get().stream()))
										.copy();
